...
| US-002 | Provision PlanningPokerAppData storage | Discover or explicitly provision the hidden SharePoint library, metadata, permissions, ODSP drive, and persisted web part configuration. | Depends on US-001 metadata decisions; all later storage uses the persisted configuration. |
...
| US-004 | Build the Microsoft 365 application shell | Replace the scaffold with a responsive Fluent UI shell, collapsible navigation, focused voting route, and About screen. | Depends on US-002 for the configuration gate; establishes shared layout and visual rules for all UI stories. |
| US-005 | Create and manage teams | Let hosts create and edit teams, rosters, activity, scale, timer, and voting-mode settings while listing only hosted teams. | Depends on US-003 and US-004; team titles must be valid unique SharePoint file names. |
