...
/**
 * Returns the durable voter IDs eligible for the current session round.
 *
 * @param session - Session whose joined roster defines eligibility.
 * @returns Stable session participant IDs in join order.
 */
...
export function selectParticipation(
  ...
  const counts: ParticipationCounts = {
    ...
    remaining: session.participants.filter((participant) => !votedIds.has(participant.id)).length
  };
